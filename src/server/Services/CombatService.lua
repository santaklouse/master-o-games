...
function CombatService.Client:FireRequest(player, payload)
	...
	if age > maxLatency then
		...
		warn(
			("[Combat] %d: fireTime %.3f s is %.0f ms old — outside the %d ms rewind window (STALE_FIRE_TIME)"):format(
				player.UserId,
				fireTime,
				age * 1000,
				Shared.Constants.LagCompensationWindowMs
			)
		)
	...
	elseif age < -FIRE_TIME_FUTURE_TOLERANCE_SECONDS then
		...
		warn(
			("[Combat] %d: fireTime %.3f s is %.0f ms in the future (FUTURE_FIRE_TIME)"):format(
				player.UserId,
				fireTime,
				-age * 1000
			)
	...
	if hit == nil then
		...
		if block ~= nil then
			warn(
				("[Combat] %d: shot stopped by world geometry %q %.1f studs out (%s)"):format(
					player.UserId,
					tostring(block.name or "World"),
					block.distance,
					reason
				)
			)
